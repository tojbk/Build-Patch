...
echo
TIME r "删除重复插件"
rm -rf ./package/gd772/luci-app-netdata
...
rm -rf package/gd772 && git clone https://github.com/281677160/openwrt-package package/gd772
...
echo
TIME y "修改主题菜单"
sed -i "s/title=VPN/title=GFW/g" feeds/luci/themes/luci-theme-design/htdocs/luci-static/design/css/style.css
...
sed -i 's/"文件管理器"/"文件管理"/g' package/gd772/luci-app-filebrowser/luasrc/controller/filebrowser.lua
...
sed -i 's/msgstr "Socat"/msgstr "端口转发"/g' package/gd772/luci-app-socat/po/zh-cn/socat.po
TIME b "重命名 完成"
echo
TIME b "菜单调整..."
